...
func flightsGetData() {
    networkManager.getData(urlString: "https://travelpayouts-travelpayouts-flight-data-v1.p.rapidapi.com/v1/prices/calendar?calendar_type=departure_date&destination=AYT&origin=SAW&depart_date=2022-11-01&currency=TRY", completionHandler: { (total: FlightsNetworkModel) in
...
func flightsGetItemData(indexPath: IndexPath) -> FlightsTableViewCellModel? {
    if let flightsArray = fligthsItemsArray?.data {
        ...
        return FlightsTableViewCellModel(name: values[indexPath.row].origin ?? "", desc: values[indexPath.row].destination ?? "",number: "\(values[indexPath.row].flight_number ?? 0)")
